...
const createService = (readOnlyValue?: string) => {
  const jwtService = {
  ...
  return { service, refreshTokenRepository, storesService };
};
...
describe('AuthService login in read-only mode', () => {
  ...
  it('issues a token pair without persisting or ensuring reports', async () => {
    const { service, refreshTokenRepository, storesService } = createService('true');
